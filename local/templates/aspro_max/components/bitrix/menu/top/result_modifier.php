...
if($arResult){
	...
	{
		$arResult[$key]['TEXT'] = preg_replace('/([0-9. ]{3,6})(.*)/', '$2', $arItem['TEXT']);
		...
		if(isset($arItem['CHILD']))
		{
			...
			{
				$arResult[$key]['CHILD'][$key2]['TEXT'] = preg_replace('/([0-9. ]{3,6})(.*)/', '$2', $arItemChild['TEXT']);
				if($bManyItemsMenu) {
				...
				foreach($arItemChild['CHILD'] as $key3 => $child){
