...
foreach ($arResult as $key => &$value) {
    $value['NAME'] = preg_replace('/([0-9. ]{3,6})(.*)/', '$2', $value['NAME']);
	if($value['CHILD']){
		foreach($value['CHILD'] as $key2 => $arChild){
			$arResult[$key]['CHILD'][$key2]['NAME'] = preg_replace('/([0-9. ]{3,6})(.*)/', '$2', $arChild['NAME']);
			if($arChild['CHILD'])
				foreach($arChild['CHILD'] as $key3 => $child){
					$arResult[$key]['CHILD'][$key2]['CHILD'][$key3]['NAME'] = preg_replace('/([0-9. ]{3,6})(.*)/', '$2', $child['NAME']);
				}
		}
	}
}